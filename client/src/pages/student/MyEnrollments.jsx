...
const MyEnrollments = () => {
  ...
  return enrolledCourses ? (
    <>
    <div className='md:px-36 px-8 pt-10 min-h-[62vh]'>
          ...
          {enrolledCourses.map((course, index)=>(
            <tr key={index} className='border-b border-gray-500/20'>
              ...
              <td className='px-4 py-3 max-sm:text-right'>
                <button className='px-3 sm:px-5 py-1.5 sm:py-2 bg-orange-600 max-sm:text-xs text-white' onClick={()=> navigate('/player/'+ course._id)}>{progressArray[index] && progressArray[index].lectureCompleted / progressArray[index].totalLectures === 1 ? 'Completed':'On Going'}</button>
              </td>
            </tr>
